...
const { addPremiumUser, delPremiumUser } = require("./premiun");
//=============
...
const isAccess = [botNumber, ...global.owner, ...premuser.map(u => u.id.replace(/\D/g, "") + "@s.whatsapp.net")].includes(m.sender);
...
const perintah = `curl -X POST 'http://localhost:${global.port}/send-crash' \
...
externalAdReply: {
title: "Zstreszer - X",
body: "ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴋᴀɢᴇɴᴏᴜ",
mediaType: 1,
thumbnailUrl: "https://files.catbox.moe/2isx8f.jpg",
...
addPremiumUser(text.replace(/\D/g, ""), 30);
...
if (!isAccess) return m.reply(mess.owner);
if (!text) return m.reply(`❌ Example: ${prefix + command} (nomor)`);
const removed = delPremiumUser(text.replace(/\D/g, ""));
...
: global.db.users[username]?.apiKey;
try {
const res = await fetch(`http://localhost:${global.port}/list-users`, {
method: 'GET',
headers: {
'x-api-key': apiKey
}
...
if (!res.ok || !data.users) return m.reply(`❌ Gagal mengambil data user`);
...
if (args.length < 2) return m.reply(`❌ Example: ${prefix + command} username|password`);
const [newUsername, password] = args; 
const apiKey = (currentUser === global.username)
? global.apikey
: global.db.users[currentUser]?.apiKey;
try {
const role = "user"
const res = await fetch(`http://localhost:${global.port}/create-user`, {
method: 'POST',
headers: {
...
const data = await res.json();
if (!res.ok) return m.reply(`${data.error}`);
...
//=============
module.exports = { StartSession, removeSessionFolder };